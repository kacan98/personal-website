...
export function getCuratedProjects(locale: Locale, siteUrl: string): CuratedProject[] {
    ...
    en: [
      {
        ...
        url: `${siteUrl}/project-stories/10x-performance-improvement`,
      },
      {
        iconName: 'schedule',
...
export async function getCVProjectsSection(locale: string = 'en'): Promise<CvSection> {
  ...
  const projects = getCuratedProjects(validLocale, settings.siteUrl);
